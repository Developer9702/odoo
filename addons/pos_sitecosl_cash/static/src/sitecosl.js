...
import {
    serializeSitecoslXml,
    parseSitecoslXml,
    makeSitecoslHeader,
    parseSitecoslResult,
    parseVerificationInfo,
} from "@pos_sitecosl_cash/utils/sitecosl_xml";
import {
    SITECOSL_STATUS,
    SITECOSL_STATUS_STRING,
    SITECOSL_CURRENCY_STATUS,
    WEBSOCKET_REQUESTS,
    XML_REQUESTS,
} from "@pos_sitecosl_cash/utils/constants";
...
export class SitecoServicioCobroService extends PaymentInterface {
    setup() {
        ...
        /**
         * @type {import("models").SiteocoslSettings}
         */
        this.settings = null;
        /**
         * @type {import("models").SitecoslUser}
         */
        this.sitecoslUser = null;
    ...
    setupWebsocket() {
        const websocketEndpoint = `${WEBSOCKET_PROTOCOL}//${this.payment_method_id.sitecosl_websocket_address}:${WEBSOCKET_PORT}${WEBSOCKET_URL}`;
        ...
        this.socketIo = new SocketIoService(websocketEndpoint, {
            ...
            onClose: () => {
                if (this.state.status !== "DISCONNECTED") {
                    ...
                    if (!document.hidden) {
                        this.showError(
                            _t(
                                "Failed to connect to Siteco cash machine, please ensure it is switched on and connected to the network."
                            )
                        );
                    }
                }
            ...
            onBinaryEvent: (data) =>
                parseSitecoslXml(data).then((response) => {
    ...
    async login() {
        if (this.payment_method_id.sitecosl_username) {
            const response = await this.sendWebsocketRequest(
                WEBSOCKET_REQUESTS.login,
                this.payment_method_id.sitecosl_username,
                this.payment_method_id.sitecosl_password
            );
            if (response.ret === 0) {
                this.sitecoslUser = response;
            }
        }
        ...
        this.waitForResponseWithType("credential ng").then(() => {
            ...
            this.showError(
                _t(
                    "Failed to login to Siteco cash machine, please check the configured username and password."
                )
            );
        });
        await this.sendWebsocketRequest(
            WEBSOCKET_REQUESTS.checkCredentials,
            this.sitecoslUser?.session_id
        );
    }
    ...
    async downloadLogs() {
        ...
        aElement.href = url;
        aElement.download = `sitecosl_logs_${luxon.DateTime.now().toFormat(
            "yyyy-LL-dd-HH-mm-ss"
        )}.txt`;
        aElement.click();
    ...
    get status() {
        return SITECOSL_STATUS_STRING[this.state.status] ?? this.state.status;
    }
    ...
    get paymentLine() {
        ...
        const sitecoslPaymentLines = order.payment_ids.filter(
            (line) => line.payment_method_id === this.payment_method_id
        );

        return sitecoslPaymentLines.find((line) =>
    ...
    statusChangeHandler(statusResponse, firstStatus = false) {
        ...
        this.state.status = SITECOSL_STATUS[statusCodeElement.textContent];
        ...
        switch (this.state.status) {
            ...
            case "COUNTING": {
                ...
                this.state.amountInserted = this.sitecoslAmountToPosAmount(amount);
                break;
            }
            case "WAITING_CANCEL": {
    ...
    parseSitecoslCashElement(cashElement) {
        ...
        return denominationElements.map((denomination) => ({
            value: this.sitecoslAmountToPosAmount(denomination.getAttribute("fv")),
            amount: parseInt(denomination.getElementsByTagName("Piece")[0].textContent),
            status: SITECOSL_CURRENCY_STATUS[
     ...
     * @returns {number}
     */
    getTotalFromSitecoslCashElement(cashElement) {
        const denominations = this.parseSitecoslCashElement(cashElement);
    ...
    inventoryChangeHandler(inventoryResponse) {
        ...
        const denominations = this.parseSitecoslCashElement(dispensableCash);
    ...
    async handlePaymentResponse(paymentResponse) {
        if (!this.paymentLine) {
            console.warn("Siteco payment response received, but no payment in progress");
            return false;
        }

        switch (parseSitecoslResult(paymentResponse)) {
            case "SUCCESS": {
    ...
    setPaymentInfo(paymentResponse, isSuccessful) {
        ...
        const cashGiven = this.getTotalFromSitecoslCashElement(cashGivenElement);
        const cashReturned = this.getTotalFromSitecoslCashElement(cashReturnedElement);
    ...
    makeReceiptMessage(transactionId, amountDeposited, amountReturned, isSuccessful) {
        const header = isSuccessful
            ? _t("SITECO TRANSACTION SUCCESSFUL")
            : _t("SITECO TRANSACTION CANCELLED");
    ...
    /**
     * @param {import("models").SitecoslRequestInfo} request
     * @param {import("models").SitecoslXmlElement[]?} children
     */
    async sendXmlRequest(request, children, secondAttempt = false) {
        const xmlElement = {
            name: request.requestName,
            children: [
                ...makeSitecoslHeader(this.sequenceNumber, this.sessionId),
                ...(children ?? []),
            ],
        };
        const xmlString = serializeSitecoslXml(xmlElement);
        ...
        const statusCode = parseSitecoslResult(xmlResponse);
    ...
    sitecoslAmountToPosAmount(amountStringInCents) {
